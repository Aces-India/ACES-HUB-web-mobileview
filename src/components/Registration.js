  ...
  const initializeTeamMembers = () => {
    return Array.from({ length: 5 }, (_, index) => {
      return {
        name: "",
        setName: (value) =>
          setTeamMembers((prev) => updateMember(prev, index, "name", value)),
        rollNo: "",
        setRollNo: (value) =>
          setTeamMembers((prev) => updateMember(prev, index, "rollNo", value)),
        gender: "M",
        setGender: (value) =>
          setTeamMembers((prev) => updateMember(prev, index, "gender", value)),
      };
    });
  };
  ...
  const handleSubmit = async () => {
    try {
      ...
      const response = await axios.post(
        "https://s-hub-backend.onrender.com/api/register",
        formData,
        {
          headers: {
            ...
            {teamMembers.map((member, index) => (
              <div key={index} className="member-container">
                <div className="member-input-container">
                  <label className="member-label">{`Team Member ${
                    index + 1
                  } Name:`}</label>
                  <input
                    type="text"
